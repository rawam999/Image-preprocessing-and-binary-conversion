...
t=T;
Rho=790; %液滴的密度
sigma=0.027; %液滴的表面张力系数
interval=4; %人为选定照片张数
gap=0;   %为读取中间图片的情况预备
cropHeight = 800;   %确定裁剪的高度
Highbegin = 2;
cropWidth = 512; 
Widthbegin = 1;
path='E:\rawam\Droplet\ethanol\1 atm\09-12-25\PROCESS\bouncing\';   %要处理的文件地址
for k=4:1:4
radii_left_all=[];
radii_right_all=[];
centers_all=[];
...
for i=1:number(k)
    ...
    [centers, radii,metric] = imfindcircles(Img4_1,[25 55],'ObjectPolarity','dark','Sensitivity',0.96); %查找液滴的质心与半径
